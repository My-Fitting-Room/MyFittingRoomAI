...
const OnboardingScreen18 = ({ navigation }) => {
    ...
    useEffect(() => {
        mixpanel.track('Onboarding screen viewed', { screen: 'OnboardingScreen18' });

        const subscription = AppState.addEventListener('change', nextAppState => {
            if (nextAppState === 'background') {
                mixpanel.track('Onboarding screen drop off screen', { screen: 'OnboardingScreen18' });
            }
        });
    ...
    const handleContinue = () => {
        if (selectedSizes.length > 0) {
            mixpanel.track('Onboarding step completed', { screen: 'OnboardingScreen18' });
